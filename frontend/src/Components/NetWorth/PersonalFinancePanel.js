...
const PersonalFinancePanel = ({ data }) => {
  const cashValue = data.cashValue ?? 0;
  const cashPct = data.cashPct ?? 0;
  const changeValue = zeroSnap(data.cashPeriodChangeValue ?? 0);

  return (
    <>
      <h3 className="networth-panel-title">Personal Finance's Contribution to Net Worth</h3>

      <div className="stats-grid">
        <div className="stats-card">
          <span className="stats-card-label">Cash Value</span>
          <span className="stats-card-value">${cashValue.toFixed(2)}</span>
        </div>
        <div className="stats-card">
          <span className="stats-card-label">% of Net Worth</span>
          <span className="stats-card-value">{cashPct.toFixed(1)}%</span>
        </div>
        <div className="stats-card">
